...
function c80800062.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFlagEffect(tp,80800062)==0 end
	Duel.RegisterFlagEffect(tp,80800062,RESET_PHASE+PHASE_END,EFFECT_FLAG_OATH,1)
end
function c80800062.filter(c)
...
function c80800062.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c80800062.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c80800062.refilter(c,cd,cod)
	return c==cd and c:IsSetCard(cod)
end
function c80800062.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c80800062.filter,tp,LOCATION_DECK,0,nil)
	...
	while sc<4 do
		...
		if	g:IsExists(c80800062.refilter,1,nil,g1:GetFirst(),cod) then
			g:Remove(Card.IsSetCard,nil,cod)
		end
		sc=sc+1
	end
	if g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(80800062,0)) then
