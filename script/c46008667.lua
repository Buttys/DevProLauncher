...
function c46008667.initial_effect(c)
	...
	e3:SetValue(c46008667.eqlimit)
	c:RegisterEffect(e3)
	--special
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(46008667,0))
	e4:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e4:SetType(EFFECT_TYPE_IGNITION)
	e4:SetRange(LOCATION_GRAVE)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetCondition(c46008667.spcon)
	e4:SetCost(c46008667.spcost)
	e4:SetTarget(c46008667.sptg)
	e4:SetOperation(c46008667.spop)
	c:RegisterEffect(e4)
end
function c46008667.eqlimit(e,c)
	return c:IsSetCard(0x107a)
end
function c46008667.eqfilter1(c)
	return c:IsFaceup() and  c:IsSetCard(0x107a)
end
function c46008667.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c46008667.eqfilter1(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c46008667.eqfilter1,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c46008667.eqfilter1,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
...
function c46008667.valcon(e,re,rp)
...
function c46008667.filter1(c,e,tp)
	...
	Duel.IsExistingMatchingCard(c46008667.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c:GetCode())
end
function c46008667.filter2(c,e,tp,code)
...
function c46008667.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,c46008667.filter1,tp,LOCATION_MZONE,0,1,1,nil,e,tp)
...
function c46008667.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c46008667.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc:GetCode())
	local sc=g:GetFirst()
	if sc then
