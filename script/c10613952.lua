...
function c10613952.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(10613952,0))
...
function c10613952.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(c10613952.filter,tp,LOCATION_GRAVE,0,1,nil,e,tp,e:GetHandler())
	end
	...
	if ft>1 and g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(10613952,2)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
		local g2=g:Select(tp,1,1,nil)
		g:Remove(Card.IsCode,nil,g2:GetFirst():GetCode())
		g1:Merge(g2)
		if ft>2 and g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(10613952,2)) then
...
function c10613952.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c10613952.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c10613952.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp)
