...
function c72497366.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(72497366,1))
	e4:SetType(EFFECT_TYPE_CONTINUOUS+EFFECT_TYPE_FIELD)
	e4:SetCode(EFFECT_DESTROY_REPLACE)
	e4:SetRange(LOCATION_SZONE)
	e4:SetTarget(c72497366.destg)
	e4:SetValue(1)
	c:RegisterEffect(e4)
end
function c72497366.ntcon(e,c)
	if c==nil then return true end
	return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 and Duel.GetFlagEffect(tp,72497366)==0
end
...
function c72497366.ntop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.RegisterFlagEffect(tp,72497366,RESET_PHASE+PHASE_END,0,1)
end
function c72497366.tg(c)
	return c:IsRace(RACE_FIEND) and c:IsLevelAbove(5)
end
function c72497366.destg(e,tp,eg,ep,ev,re,r,rp,chk)
