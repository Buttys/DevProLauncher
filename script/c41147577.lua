...
function c41147577.initial_effect(c)
	...
	e2:SetCondition(c41147577.damcon)
...
function c41147577.filter(c)
	return not c:IsDisabled()
end
function c41147577.postg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) end
	if chk==0 then return Duel.IsExistingTarget(c41147577.filter,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,c41147577.filter,tp,0,LOCATION_MZONE,1,1,nil)
