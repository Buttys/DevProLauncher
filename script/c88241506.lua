...
function c88241506.initial_effect(c)
	...
	e1:SetOperation(c88241506.ngattack)
	c:RegisterEffect(e1)
	--be target
...
function c88241506.filter(c,e,tp)
	return c:GetCode()==89631139 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c88241506.ngattack(e,tp,eg,ep,ev,re,r,rp)
	if Duel.NegateAttack() then
		...
		if c:IsFaceup() and c:IsRelateToEffect(e) then
			...
			if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
				...
				local g=Duel.SelectMatchingCard(tp,c88241506.filter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c88241506.spcon(e,tp,eg,ep,ev,re,r,rp,chk)
